...
def tabulate_scores(X_train, y_train, X_test, y_test ,flag=False):
    ...
    eclf = VotingClassifier(estimators=[('dt', clf_decision_tree), ('knn', clf_nearest_neighbor),
                                        ...
                                        ('rf', clf_random_forest)], voting='soft', weights=[1, 1, 2, 2, 3, 2])
    ...
    # Adding Random Forest
    print_scores_clf(clf_random_forest, X_test, y_test, 'Random Forest', skf)
    ...
    if flag:
        ...
        # Random forest
        clf6 = RandomForestClassifier(n_estimators=10, random_state=0)
        eclf = VotingClassifier(estimators=[('dt', clf_decision_tree), ('knn', clf_nearest_neighbor),
                                             ('mlp', clf_neural_network), ('gnb', clf_gaussian_naive_bayes),
        ...
        # Random forest
        clf6.fit(X_presentation, y)
        eclf.fit(X_presentation, y)

        classifiers = [clf1, clf2, clf3, clf4, clf5, clf6, eclf]
        titles = ['Decision Tree (depth=10)', 'KNN (k=8)', 'GNB', 'Neural Network', 'Linear Kernel SVM',
        ...
        fig, axes = plt.subplots(3, 3, figsize=(15, 12))  # Adjust as needed for the number of classifiers
        axes = axes.flatten()
        ...
        for idx, (clf, title) in enumerate(zip(classifiers, titles)):
            ...
            ax.scatter(X_presentation[:, 0], X_presentation[:, 1], c=y, alpha=0.8)
            ax.set_title(title)

        p1 = Circle((0, 0), fc="yellow")
        p2 = Circle((0.2, 0), fc="purple")
        p3 = Circle((0, 0), fc="lightblue")
        fig.legend([p1, p2, p3], ['TYP', 'SLI', 'ASD'], loc='upper right')
...
if __name__ == "__main__": # main
    ...
    tabulate_scores(X_train, y_train_, X_test, y_test_)
